...
- **Нативне захоплення системного звуку** — Process Tap на macOS 14.4+; BlackHole лишається автоматичним резервним маршрутом
...
- **Офлайн SQLite + мережевий fallback** — Миттєвий пошук у локальній базі з резервним зверненням до GitHub
...
- [BlackHole 2ch](https://github.com/ExistentialAudio/BlackHole) (необов’язковий резервний маршрут для macOS 13–14.3 або ручного режиму BlackHole)
...
3. **Перший запуск (один зі способів):**
   ...
   - В терміналі:
     ...
     ```
4. На macOS 14.4+ залиш **Audio Backend** у режимі **Автоматичний**. Встановлюй BlackHole лише коли SystemEQ запропонує резервний маршрут або якщо обереш режим BlackHole вручну.
...
1. **Використовуй автоматичну маршрутизацію** (рекомендовано на macOS 14.4+):
   - Відкрий SystemEQ → Settings → Audio Backend
   - Залиш режим **Автоматичний**
   - Обери навушники або колонки в Routing і натисни **Увімкнути EQ**
   - SystemEQ захоплює звук нативно; системний вихід macOS змінювати не треба

2. **Використовуй BlackHole лише за потреби** (macOS 13–14.3, fallback або ручний вибір):
   - Встанови [BlackHole 2ch](https://existential.audio/blackhole/) і перезапусти SystemEQ
   - У Routing обери BlackHole як вхід, а навушники або колонки — як вихід
   - Встанови **BlackHole 2ch** системним виходом macOS і натисни **Увімкнути EQ**
...
```text
macOS 14.4+: Системний звук → Process Tap → vDSP Biquad EQ → Фізичний вихід

Резервний:   Системний звук → BlackHole 2ch → vDSP Biquad EQ → Фізичний вихід
```

**Multi-Output Device не потрібен.** Автоматичний режим використовує нативний рушій macOS, коли він доступний; BlackHole лишається для сумісності та резервного переходу.
...
На macOS 14.4+ спершу обери **Автоматичний** у Settings → Audio Backend і ще раз увімкни EQ. Системний вихід має лишатися фізичним пристроєм. Якщо Automatic перейшов у fallback або не запускається, встанови BlackHole і виконай інструкції нижче.
